...
console.log("g1a", fenomenler[0].profile);

//(2) Dizideki üçüncü fenomenin (2. dizin) takipçi (followers) sayısı
console.log("g1b", fenomenler[2].followers);
...
console.log("g2", fenomenler[6].profile);
/*  Görev 3:
...
console.log("g3", indekseGoreFenomen(fenomenler, 3));
...
function enFenomenler(arr) {
  ...
  for (const element of arr) {
    element.followers > 100000000 && mostFeno.push(element.profile);
  }
  return mostFeno;
}
...
function fenomenGonderimSayisi(arr, name) {
  for (const element of arr) {
    if (element.profile == name) return element.posts;
  }
}

console.log("g8", fenomenGonderimSayisi(fenomenler, "Will Smith"));
/* Görev 9:
...
console.log("g9", platformaGoreCokGonderiYapanFenomen(fenomenler, "TikTok"));
/* ***** GÖREVLERİN SONU ***** */
